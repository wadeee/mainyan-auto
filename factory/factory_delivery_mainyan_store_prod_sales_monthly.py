...
用法：
    python factory_delivery_mainyan_store_prod_sales_monthly.py                    # 导出本月数据
    python factory_delivery_mainyan_store_prod_sales_monthly.py --months -1        # 导出上月数据
    python factory_delivery_mainyan_store_prod_sales_monthly.py --date 2026.06.03  # 根据日期推断所在月
    python factory_delivery_mainyan_store_prod_sales_monthly.py --headless          # 无头模式（不显示浏览器）
"""
...
_file_handler = logging.handlers.TimedRotatingFileHandler(
    LOG_DIR / "factory_delivery_mainyan_store_prod_sales_monthly.log",
    when="midnight",
    backupCount=30,
    encoding="utf-8",
)
...
OUTPUT_DIR = Path(__file__).resolve().parent / "工厂配送麦安研+焙满香门店商品月度销售对比表"
TEMPLATE_FILE = Path(__file__).resolve().parent / "工厂配送麦安研+焙满香门店商品月度销售对比表_格式化模板.xlsx"
...
def get_month_range(months=0, date_str=None):
    if date_str:
...
def _update_rich_text_date(cell, first_day, last_day):
...
def fill_s1(wb, sorted_products, store_order, first_day, last_day):
    ws = wb.worksheets[0]
    ...
    _update_rich_text_date(ws.cell(row=1, column=1), first_day, last_day)


# ─── Sheet 2：货品销售排行表 ───────────────────────────────────────────────────


def fill_s2(wb, sorted_products, first_day, last_day):
    ws = wb.worksheets[1]
    s2_max_col = 8
...
def fill_s3_sheets(wb, sorted_products, store_order, first_day, last_day, brand_map=None):
    ...
    for idx, short_name in enumerate(store_order):
        ...
        _fill_single_s3(ws, store_products, short_name, first_day, last_day, brand=brand)


def _fill_single_s3(ws, store_products, short_name, first_day, last_day, brand="麦安研"):
    s3_max_col = 8
...
def merge_into_template(data_file, template_file, output_file, first_day, last_day, bmx_data=None):
    logger.info("读取下载数据...")
    ...
    fill_s1(wb, sorted_products, all_store_order, first_day, last_day)

    logger.info("填充 Sheet 2 (货品销售排行表)...")
    s2_sorted = sorted(active_products, key=lambda p: -sum((s.get("金额") or 0) for s in p["stores"].values()))
    fill_s2(wb, s2_sorted, first_day, last_day)

    logger.info(f"填充 Sheet 3+ ({len(all_store_order)} 个门店销售排行)...")
    fill_s3_sheets(wb, sorted_products, all_store_order, first_day, last_day, brand_map)
...
def main():
    parser = argparse.ArgumentParser(description="Pospal 工厂配送麦安研+焙满香门店商品月度销售对比表自动化脚本")
    ...
    logger.info(f"{'=' * 55}")
    logger.info(f"  Pospal 工厂配送麦安研+焙满香门店商品月度销售对比表")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            logger.info(f"  → 设置时间: {first_day_str} ~ {last_day_str}...")
            ...
            formatted_output = OUTPUT_DIR / date_range_str / f"工厂配送麦安研+焙满香门店商品月度销售对比表_{date_range_str}.xlsx"
            merge_into_template(dest, TEMPLATE_FILE, formatted_output, first_day, last_day, bmx_data)

            logger.info(f"{'=' * 55}")
            logger.info(f"  工厂配送麦安研+焙满香门店商品月度销售对比表全部完成！")
