...
def _fill_single_s3(ws, store_products, short_name, monday, sunday):
    s3_max_col = 8
    ...
    if cell_a1.value:
        if isinstance(cell_a1.value, CellRichText):
            ...
            new_text = re.sub(r'麦安研\S+门店', f'麦安研{short_name}门店', new_text)
            cell_a1.value = new_text
        else:
            new_val = re.sub(date_pattern, date_replacement, str(cell_a1.value))
            new_val = re.sub(r'麦安研\S+门店', f'麦安研{short_name}门店', new_val)
...
def merge_into_template(data_file, template_file, output_file, monday, sunday):
    logger.info("读取下载数据...")
    ...
    logger.info(f"填充 Sheet 1 (货品对比表), {len(sorted_products)} 条商品, {len(store_order)} 个门店...")
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            merge_into_template(dest, TEMPLATE_FILE, formatted_output, monday, sunday)
