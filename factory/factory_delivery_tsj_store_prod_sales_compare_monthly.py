"""
工厂配送兔司家门店商品月度销售对比表 - 自动化脚本
=====================================
依赖：pip install playwright && playwright install chromium

自动登录 Pospal 后台，依次完成工厂配送兔司家门店商品月度销售对比表所需的数据导出与处理。

用法：
    python factory_delivery_tsj_store_prod_sales_compare_monthly.py                    # 导出本月数据
    python factory_delivery_tsj_store_prod_sales_compare_monthly.py --months -1        # 导出上月数据
    python factory_delivery_tsj_store_prod_sales_compare_monthly.py --date 2026.06.03  # 根据日期推断所在月
    python factory_delivery_tsj_store_prod_sales_compare_monthly.py --headless          # 无头模式（不显示浏览器）
"""
...
_file_handler = logging.handlers.TimedRotatingFileHandler(
    LOG_DIR / "factory_delivery_tsj_store_prod_sales_compare_monthly.log",
    when="midnight",
    backupCount=30,
    encoding="utf-8",
)
...
OUTPUT_DIR = Path(__file__).resolve().parent / "工厂配送兔司家门店商品月度销售对比表"
TEMPLATE_FILE = Path(__file__).resolve().parent / "工厂配送兔司家门店商品月度销售对比表_格式化模板.xlsx"
...
def main():
    parser = argparse.ArgumentParser(description="Pospal 工厂配送兔司家门店商品月度销售对比表自动化脚本")
    ...
    logger.info(f"{'=' * 55}")
    logger.info(f"  Pospal 工厂配送兔司家门店商品月度销售对比表")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            logger.info(f"{'=' * 55}")
            logger.info(f"  工厂配送兔司家门店商品月度销售对比表下载完成！")
            ...
            formatted_output = OUTPUT_DIR / date_range_str / f"工厂配送兔司家门店商品月度销售对比表_{date_range_str}.xlsx"
            ...
            logger.info(f"{'=' * 55}")
            logger.info(f"  工厂配送兔司家门店商品月度销售对比表全部完成！")
            logger.info(f"{'=' * 55}\n")
